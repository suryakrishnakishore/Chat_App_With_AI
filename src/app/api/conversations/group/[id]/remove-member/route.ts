...
export async function PATCH(req: Request, context: { params: { id: string }}) {
    ...
    if(!groupConversation.admins.includes(user.user_id)) {
        return NextResponse.json({ error: "Only admins can remove members from the group." }, { status: 403 });
    }

    if(!memberIds || !Array.isArray(memberIds)) {
        return NextResponse.json({ error: "Member IDs are required to remove members." }, { status: 400 });
    }

    const uniqueIds = memberIds.filter((memberId: string) => groupConversation.participants.includes(memberId));
    groupConversation.participants = groupConversation.participants.filter((participantId: string) => !uniqueIds.includes(participantId));
